...
def main(start=0, stop=2000, content='gee_ims'):
    ...
    path_csv_locations =  '/Users/tplas/data/dw_sampled_locations/data_spherical/downstream_tasks/cropharvest_cleaned_global_threshold-200-sample.csv'
    ...
    name_list = ['cropharvest_' + str(i) for i in df_locations['index'].values]
    ...
    if content == 'gee_ims':
        inds_none = gu.download_list_coord(coord_list=coords_list, name_list=name_list, path_save=folder_save, bool_buffer_in_deg=False, buffer_deg=None, buffer_m=800,
                           start_index=start, stop_index=stop, resize_image=True, threshold_size=128,
                           list_collections=['alphaearth'], save_coords_json=False)
